...
public class GrafoDirigido<V, D> implements IDirectedIGraph<V, D> {
    ...
    public boolean agregarVertice(V vertex) {
        ...
        vertices.add(vertex);
        adyacencia.put(vertex, new ArrayList<>());
        return true;
    }
    ...
    public boolean agregarArista(V source, V target, D dato) {
        ...
        adyacencia.get(source).add(new DirectedEdge<>(source, target, dato));
        return true;
    }
    ...
    public boolean eliminarArista(Comparable<V> source, Comparable<V> target) {
        ...
        return adyacencia.get(src).removeIf(e -> target.compareTo(e.target()) == 0);
    }
    ...
    public Set<Edge<V, D>> aristas() {
        ...
        adyacencia.values().forEach(result::addAll);
    ...
    public List<Edge<V, D>> adyacencias(Comparable<V> verticeCriteria) {
        ...
        return Collections.unmodifiableList(adyacencia.get(vertex));
    }
    ...
    public Set<V> successors(Comparable<V> criteria) {
        ...
        adyacencia.get(vertex).forEach(e -> result.add(e.target()));
        return result;
    }
